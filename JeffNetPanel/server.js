...
import { kpis, Naps, Outages, Reports, CSessions, ClientStatus, Sms, Routers } from "./lib/db.js";
...
function paymentGateway() {
  ...
  return {
    ...
    createLink: (opts) => pmCreateLink({ secret: s.paymongo_secret || process.env.PAYMONGO_SECRET || "", baseUrl: s.paymongo_base || undefined }, opts),
...
function connForCustomer(c) {
  ...
  return mt;   // no routers configured → legacy single-router behavior
}
  ...
  try {
    ...
    if (pathname === "/api/billing/xendit/webhook" && req.method === "POST") {
      ...
      if (ev && ev.paid) {
        ...
        if (!inv && ev.remarks) { const mm2 = String(ev.remarks).match(/INV-(\d+)/); if (mm2) inv = Invoices.get(Number(mm2[1])); }
        if (inv && inv.status !== "paid") {
          Invoices.pay(inv.id, { method: "gcash", reference: ev.resourceId || "xendit", note: "Xendit online payment" });
          Audit.add({ type: "auto", customer_id: inv.customer_id, customer_name: inv.customer_name, action: "invoice-paid-online", detail: ev.resourceId || "", ok: true });
          try {
            const cust = Customers.get(inv.customer_id);
            if (cust && cust.status === "suspended") {
    ...
    if (pathname === "/pay" && req.method === "GET") {
      // /pay now uses the unified account page (login + top-up/renew/pay)
      res.writeHead(302, { Location: "/account" });
      return res.end();
    }
    ...
    if (pathname === "/api/plans" && req.method === "GET") {
      const s = Settings.all();
      const plans = (Plans.list() || []).map((p) => ({
        id: p.id, name: p.name, price: p.price, speed: p.speed,
        validity_days: p.validity_days, type: p.type || "pppoe", features: p.features || "",
      }));
      const config = {
        biz_name: s.biz_name || "JEFF NETWORK SERVICE",
    ...
    // My Account lookup: by account code OR username, plus last name.
    if (pathname === "/api/myaccount" && req.method === "POST") {
      ...
      const cust = (Customers.list() || []).find((c) => {
        ...
        const lastName = String(c.name || "").trim().split(/\s+/).pop().toLowerCase();
        return lastName === last || String(c.name || "").toLowerCase().includes(last);
      });
      if (!cust) return send(res, 404, { ok: false, error: "No match. Check your account code or username and last name, or contact us." });
      const account = {
    ...
    // Start a real online payment via PayMongo (returns a checkout URL the customer opens).
    if (pathname === "/api/pay-start" && req.method === "POST") {
      ...
      const cust = (Customers.list() || []).find((c) => {
        const codes = [c.account_code, c.username].map((x) => String(x || "").toLowerCase());
        if (!codes.includes(code)) return false;
        const lastName = String(c.name || "").trim().split(/\s+/).pop().toLowerCase();
        return lastName === last || String(c.name || "").toLowerCase().includes(last);
      });
      if (!cust) return send(res, 404, { ok: false, error: "Account not found. Check your code and last name." });
      ...
      try {
        const link = await gw.createLink(
          { amountPhp: amount, description: (purpose === "topup" ? "Wallet top-up" : "Plan " + purpose) + " — " + cust.name, remarks: (cust.account_code || cust.username || "") + ":" + purpose }
        );
        return send(res, 200, { ok: true, url: link.checkout_url || link.url, id: link.id });
      } catch (e) {
  ...
  try {
    switch (sub) {
      ...
      case "/hotspot/vouchers": {
        ...
        // Per-router: if body.router_id is given, create on that router; else the global/default.
        let vconn = mt, vRouterName = "";
        if (b.router_id) {
          const vr = Routers.get(Number(b.router_id));
          if (!vr) return send(res, 404, { ok: false, error: "Router not found." });
          try { vconn = connForRouter(vr); vRouterName = vr.name; await vconn.identity(); } catch (e) { return send(res, 502, { ok: false, error: "Can't reach " + vr.name + ": " + e.message }); }
        }
        const made = [], errors = [];
        for (let i = 0; i < count; i++) {
          const code = genVoucherCode(b.length, b.prefix);
          const password = userOnly ? "" : code;
          try { await vconn.createHotspotUser({ name: code, password, profile, limitUptime: b.uptime || "" }); made.push(code); }
          catch (e) { errors.push(e.message); }
        }
        Audit.add({ type: "manual", action: "vouchers", detail: `${made.length}x ${profile}${userOnly ? " (user-only)" : ""}${vRouterName ? " @ " + vRouterName : ""}`, ok: true });
        return ok({ created: made, count: made.length, profile, uptime: b.uptime || "", price: b.price || "", exp: b.exp || "", userOnly, errors, router: vRouterName, dryRun: !!RouterOSAPI.dryRun });
      }
  ...
  try {
    ...
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-profiles$/)) && method === "GET") {
      ...
      try {
        ...
        return ok((profs || []).map((p) => ({ name: p.name })));
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    ...
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-user$/)) && method === "POST") {
      ...
      try {
        const conn = connForRouter(r);
        await conn.createHotspotUser({ name: body.name, password: body.password || "", profile: body.profile || "default", limitUptime: body.uptime || "" });
        Audit.add({ type: "manual", action: "hotspot-user-add", detail: `${body.name} (${body.profile || "default"}) @ ${r.name}`, ok: true });
        return ok({ ok: true, name: body.name, dryRun: !!RouterOSAPI.dryRun });
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    ...
    if ((mm = m(/^\/routers\/(\d+)\/live$/)) && method === "GET") {
      ...
      try {
        ...
        try {
          const hs = (await conn.listHotspot ? await conn.listHotspot() : []) || [];
    ...
    if (sub === "/routers" && method === "POST") {
      ...
      // License tier: cap how many routers this install may have.
      const maxR = License.maxRoutersFor(LICENSE_STATE.license);
      if (maxR > 0) {
        const have = Routers.list().length;
        if (have >= maxR) return send(res, 403, { ok: false, error: `Your license allows up to ${maxR} router${maxR > 1 ? "s" : ""} (tier: ${License.tierLabel(LICENSE_STATE.license)}). You already have ${have}. Upgrade your license to add more sites.` });
      }
    ...
    if ((mm = m(/^\/routers\/(\d+)\/reprovision$/)) && method === "POST") {
      ...
      for (const c of custs) {
        try {
          ...
          const profile = (c.status === "suspended" ? (Settings.get("suspended_profile") || (c.plan_profile || "default")) : (c.plan_profile || "default"));
    ...
    if (sub === "/settings" && method === "GET") {
      const s = Settings.all();
      for (const k of ["smtp_pass", "paymongo_secret", "paymongo_webhook_secret", "xendit_secret", "xendit_callback_token", "telegram_bot_token", "semaphore_api_key", "mikrotik_password", "ai_api_key", "portal_token"]) if (s[k]) s[k] = "***";
      return ok(s);
    }
    if (sub === "/settings" && method === "POST") {
      const patch = { ...body };
      for (const k of ["smtp_pass", "paymongo_secret", "paymongo_webhook_secret", "xendit_secret", "xendit_callback_token", "telegram_bot_token", "semaphore_api_key", "mikrotik_password", "ai_api_key", "portal_token"])
...
async function pushPortalConfig() {
  ...
  try {
    ...
    const config = {
      ...
      biz_name: Settings.get("biz_name", "") || "JEFF NETWORK SERVICE",
...
async function runCollections({ dryRun = false, graceDays = 0 } = {}) {
  ...
  for (const c of Collections.toSuspend(graceDays)) {
    ...
    try {
      const conn = connForCustomer(c);
      if (isHotspot(c)) await conn.setHotspotUserDisabled(c.username, true);
      else await conn.setPppoeDisabled(c.username, true);
      Customers.setStatusAndAuto(c.id, "suspended", true);
  ...
  for (const c of Collections.toReconnect()) {
    ...
    try {
      const conn = connForCustomer(c);
      if (isHotspot(c)) await conn.setHotspotUserDisabled(c.username, false);
      else await conn.setPppoeDisabled(c.username, false);
...
async function ipoeProvision(c) {
  ...
  try {
    await conn.leaseAddStatic({ address: c.static_ip, mac, server, rateLimit: rl, comment: "ipoe " + c.name });
  } catch (e) {
  ...
  try { await conn.addrListRemove(IPOE_SUSPEND_LIST, c.static_ip); } catch {}
}
async function ipoeSuspend(c) {
  ...
  await conn.addrListAdd(IPOE_SUSPEND_LIST, c.static_ip, "suspended " + c.name);
...
async function ipoeEnable(c) {
  ...
  const conn = connForCustomer(c);
  await conn.addrListRemove(IPOE_SUSPEND_LIST, c.static_ip);
...
async function applyProfileSwitch(c, target) {
  ...
  if (target === "suspend") {
    if (suspProfile) { if (isHotspot) await conn.updateHotspotUser(c.username, { profile: suspProfile }); else await conn.updatePppoe(c.username, { profile: suspProfile }); }
    else { if (isHotspot) await conn.setHotspotUserDisabled(c.username, true); else await conn.setPppoeDisabled(c.username, true); }
  } else {
    if (suspProfile) { if (isHotspot) await conn.updateHotspotUser(c.username, { profile: activeProfile }); else await conn.updatePppoe(c.username, { profile: activeProfile }); }
    else { if (isHotspot) await conn.setHotspotUserDisabled(c.username, false); else await conn.setPppoeDisabled(c.username, false); }
  }
...
async function moveCustomerToRouter(id, newRouterId, opts = {}) {
  ...
  try {
    ...
    if (isIpoe) { await ipoeProvision(fresh); }
      ...
      const profile = (fresh.status === "suspended" ? (Settings.get("suspended_profile") || (fresh.plan_profile || "default")) : (fresh.plan_profile || "default"));
