  ...
  try {
    ...
    if (pathname === "/api/pay/renew-wallet" && req.method === "POST") {
      ...
      try {
        ...
        Customers.addCredit(cust.id, price, "refund: wallet renewal failed"); // refund on failure
...
async function handleBilling(req, res, pathname) {
  ...
  try {
    ...
    if (sub === "/settings" && method === "GET") {
      const s = Settings.all();
      for (const k of ["smtp_pass", "paymongo_secret", "paymongo_webhook_secret", "telegram_bot_token", "semaphore_api_key", "mikrotik_password", "ai_api_key"]) if (s[k]) s[k] = "***";
      return ok(s);
    }
    if (sub === "/settings" && method === "POST") {
      const patch = { ...body };
      for (const k of ["smtp_pass", "paymongo_secret", "paymongo_webhook_secret", "telegram_bot_token", "semaphore_api_key", "mikrotik_password", "ai_api_key"])
    ...
    if ((mm = m(/^\/joborders\/(\d+)\/release-unit$/)) && method === "POST") {
      ...
      try {
        ...
        if (unitRow && unitRow.item_id) {
          Inventory.logMove({ item_id: unitRow.item_id, type: "out", qty: 1, customer_id: jo.customer_id || null, install_id: installId, tech: jo.tech || "", note: `released to JO #${jo.id} (${jo.name || ""})`.trim() });
        }
...
server.listen(PORT, () => {
  ...
  console.log("============================================");
  if (process.platform === "win32") exec(`start "" "${url}"`); // auto-open browser
