...
export class MikroTik {
  ...
  request(method, path, body) {
    // Dry-run: block write methods (POST/PUT/PATCH/DELETE) so they're logged but not sent,
    // matching the binary-API behavior. Reads (GET) always go through.
    if (MikroTik.dryRun && /^(POST|PUT|PATCH|DELETE)$/i.test(method)) {
      const human = method + " /rest" + path + (body ? " " + JSON.stringify(body) : "");
      try { (MikroTik.onDryRun || (() => {}))(human); } catch {}
  ...
  arpTable() { return this.request("GET", "/ip/arp"); }
  async pingHost(address, count = 2) {
    try {
      const rows = await this.request("POST", "/ping", { address, count: String(count) });
      const last = (rows || [])[(rows || []).length - 1] || {};
      const received = Number(last.received || 0);
      return { reachable: received > 0, received, sent: count };
  ...
  async deletePppProfile(name) {
    ...
    const id = await this.findId("/ppp/profile", "name", name);
    if (!id) throw new Error(`PPP profile "${name}" not found`);
    return this.request("DELETE", `/ppp/profile/${id}`);
  }
  ...
  async snapshot() {
    const sections = {
      ...
      pingHost:              (a, c) => this.pingHost(a, c),
      radius:                () => this.radius(),
