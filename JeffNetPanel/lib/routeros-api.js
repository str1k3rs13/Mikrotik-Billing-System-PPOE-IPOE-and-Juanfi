...
export class RouterOSAPI {
  ...
  async deletePppProfile(name) {
    if (String(name).toLowerCase() === "default") throw new Error('Cannot delete the "default" profile.');
    const id = await this.findId("/ppp/profile", "name", name);
    if (!id) throw new Error(`PPP profile "${name}" not found`);
    return this.removeById("/ppp/profile", id);
  }
  ...
  arpTable() { return this.print("/ip/arp"); }
  ...
  // Ping a host FROM the router (count packets) — used to check if a NodeMCU/vendo is reachable.
  async pingHost(address, count = 2) {
    try {
      const rows = await this.talk(["/ping", "=address=" + address, "=count=" + count]);
      // RouterOS /ping streams one !re row per packet. A reply that got through has a "time"
      // field (round-trip time) and/or a non-zero "received". Check ALL rows, not just the last,
      // because the final row may be a summary or a timeout row with received reset.
      let gotReply = false, maxReceived = 0;
      for (const r of (rows || [])) {
        if (r.time != null && String(r.time) !== "") gotReply = true;       // a round-trip time = host answered
        const rcv = Number(r.received || 0);
        if (rcv > maxReceived) maxReceived = rcv;
        if (rcv > 0) gotReply = true;
      }
      return { reachable: gotReply, received: maxReceived, sent: count };
    } catch (e) { return { reachable: false, received: 0, sent: count, error: e.message }; }
  }
