...
MCowBQYDK2VwAyEAGYlqysa/XQNZ4+sktKcAAcL2ZMYKdjGohWNd8CN5FfI=
-----END PUBLIC KEY-----`;
...
export function checkLicense(baseDir) {
  // A real installed license ALWAYS takes priority. The dev escape hatch must never override or
  // mask a customer's actual license (that caused real licenses to display as "DEV MODE").
  const { obj } = loadLicenseFile(baseDir);
  if (obj) {
    const r = verifyLicense(obj);
    return { ...r, machineId: machineId() };
  }
  // No license file present. Allow the dev/escape hatch ONLY when explicitly enabled for local dev.
  // This never activates in a shipped build unless the operator themselves set LICENSE_DISABLED=1.
  if (process.env.LICENSE_DISABLED === "1") {
    return { ok: true, license: { model: "dev", customer: "DEV MODE", tier: "unlimited" }, machineId: machineId(), dev: true };
  }
  return { ok: false, reason: "no-license", machineId: machineId() };
}

// ===== Layer 5: router-count tiers =====
// A license's tier (in data.tier) maps to how many routers the install may have.
// "unlimited" or missing tier → no limit (back-compat: old licenses keep working unrestricted).
export const TIERS = { solo: 1, starter: 3, pro: 10, unlimited: 0 };  // 0 = unlimited

// Resolve the max routers a license allows. Reads data.tier (preferred) or data.maxRouters.
// Returns 0 for unlimited / unknown / missing (fail-open so existing licenses aren't suddenly capped).
export function maxRoutersFor(license) {
  if (!license) return 0;
  if (license.maxRouters != null) { const n = Number(license.maxRouters); return Number.isFinite(n) && n > 0 ? n : 0; }
  const t = String(license.tier || "").toLowerCase();
  if (t && t in TIERS) return TIERS[t];
  // DEV mode / no tier → unlimited
  return 0;
}
export function tierLabel(license) {
  if (!license) return "unlimited";
  const t = String(license.tier || "").toLowerCase();
  if (t && t in TIERS) return t;
  if (license.maxRouters) return license.maxRouters + " routers";
  return "unlimited";
}
