...
export const Plans = {
  ...
  create: (p) => {
    ...
    const r = run(
      "INSERT INTO plans (name,price,speed,validity_days,validity_mins,router_profile,type,data_cap_gb,features) VALUES (?,?,?,?,?,?,?,?,?)",
      p.name, Number(p.price) || 0, p.speed || "", Math.max(1, Math.round(mins / 1440)), mins,
      p.router_profile || "default", p.type || "pppoe", Number(p.data_cap_gb) || 0, p.features || ""
    );
  ...
  update: (id, p) => {
    const mins = Number(p.validity_mins) || (Number(p.validity_days) || 30) * 1440;
    run(
      "UPDATE plans SET name=?,price=?,speed=?,validity_days=?,validity_mins=?,router_profile=?,type=?,data_cap_gb=?,features=? WHERE id=?",
      p.name, Number(p.price) || 0, p.speed || "", Math.max(1, Math.round(mins / 1440)), mins,
      p.router_profile || "default", p.type || "pppoe", Number(p.data_cap_gb) || 0, p.features != null ? p.features : "", id
    );
...
export const Customers = {
  list: () =>
    all(`SELECT c.*, p.name AS plan_name, p.price AS plan_price, p.type AS plan_type,
         ...
         FROM customers c LEFT JOIN plans p ON p.id=c.plan_id LEFT JOIN routers rt ON rt.id=c.router_id
         ORDER BY c.name ASC`),
  get: (id) =>
    get(`SELECT c.*, p.name AS plan_name, p.price AS plan_price, p.type AS plan_type,
                p.router_profile AS plan_profile, p.validity_days AS plan_days, p.validity_mins AS plan_mins, p.data_cap_gb AS plan_cap, p.speed AS plan_speed,
                rt.name AS router_name
         FROM customers c LEFT JOIN plans p ON p.id=c.plan_id LEFT JOIN routers rt ON rt.id=c.router_id WHERE c.id=?`, id),
  create: (c) => {
    ...
    const r = run(
      `INSERT INTO customers (name,contact,address,area,username,password,plan_id,billing_day,status,notes,lat,lng,conn_type,mac,static_ip,vlan_iface,account_code,router_id)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
  ...
  update: (id, c) => {
    ...
    run(
      `UPDATE customers SET name=?,contact=?,address=?,area=?,username=?,password=?,plan_id=?,billing_day=?,notes=?,conn_type=?,mac=?,static_ip=?,vlan_iface=?,router_id=? WHERE id=?`,
      c.name != null ? c.name : (cur ? cur.name : ""), c.contact || "", c.address || "", c.area || "", c.username || "",
      (c.password != null && String(c.password) !== "") ? c.password : (cur ? cur.password : ""),
      c.plan_id || null, Number(c.billing_day) || 1, c.notes || "",
      c.conn_type || "pppoe", (c.mac || "").toUpperCase(), c.static_ip || "", c.vlan_iface || "",
      (c.router_id != null && c.router_id !== "") ? Number(c.router_id) : (cur ? cur.router_id : null), id
    );
...
export const Reports = {
  ...
  cashflow: (period) => {
    ...
    const subs = sumWhere("COALESCE(note,'') NOT LIKE 'Install:%' AND COALESCE(note,'') NOT LIKE 'Hardware:%' AND COALESCE(method,'') <> 'credit' AND NOT (COALESCE(method,'')='wallet' AND COALESCE(note,'') NOT LIKE 'Wallet topup:%')");
