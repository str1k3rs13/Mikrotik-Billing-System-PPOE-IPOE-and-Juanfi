...
export const Invoices = {
  ...
  pay: (id, pay = {}) => {
    ...
    if (toCredit > 0 && inv.customer_id) Customers.addCredit(inv.customer_id, toCredit, "overpayment on invoice #" + id);
...
export const Inventory = {
  ...
  addItem: (i) => {
    const r = run("INSERT INTO inventory_items (name,category,unit,qty,reorder_level,cost,sell_price,notes,serialized) VALUES (?,?,?,?,?,?,?,?,?)",
      i.name, i.category || "", i.unit || "pcs", Number(i.qty) || 0, Number(i.reorder_level) || 0, Number(i.cost) || 0, Number(i.sell_price) || 0, i.notes || "", i.serialized ? 1 : 0);
    const item = Inventory.item(r.lastInsertRowid);
    if (Number(i.qty) > 0) run("INSERT INTO inventory_moves (item_id,type,qty,note) VALUES (?,?,?,?)", item.id, "in", Number(i.qty), "opening stock");
    return item;
  },
  updateItem: (id, i) => {
...
export const JobOrders = {
  ...
  apply: (a) => {
    const r = run(`INSERT INTO job_orders (name,contact,email,address,area,lat,lng,plan_id,conn_type,notes,install_fee,router_cost,pay_choice,pay_status,pay_reference,pay_proof,agreed,status)
      ...
      a.pay_choice === "now" && (a.pay_reference || a.pay_proof) ? "proof_submitted" : "unpaid",
...
export const Reports = {
  ...
  monthlyStatement: (period) => {
    ...
    const income = {
      ...
      hardware: cf.in.hardwareMargin,        // router/equipment resale margin
  ...
  cashflow: (period) => {
    ...
    // subscriptions = all payments that are NOT install/hardware tagged
    const subs = sumWhere("COALESCE(note,'') NOT LIKE 'Install:%' AND COALESCE(note,'') NOT LIKE 'Hardware:%'");
    ...
    const moneyIn = inSubs + inInstall + inHwMargin;
